...
namespace PayrollWeb.Utility
{
    public class CommonDateClass
    {
        ...
        static int[] monthDaysAsThirty = {30, 30, 30, 30, 30, 30, 
                                    30, 30, 30, 30, 30, 30};
        ...
        public static int DateDiffernceWithLeapYr(DateTime FromDate, DateTime ToDate)
        { 
           // COUNT TOTAL NUMBER OF DAYS  
            // BEFORE FIRST DATE 'FromDate' 
  
         // initialize count using years and day 
            int n1 = FromDate.Year * 365 + FromDate.Day; 
  
                // Add days for months in given date 
            for (int i = 0; i < FromDate.Month - 1; i++)  
                { 
                    n1 += monthDays[i]; 
                } 
                ...
                n1 += countLeapYears(FromDate); 
  
                // SIMILARLY, COUNT TOTAL  
                // NUMBER OF DAYS BEFORE 'ToDate' 
                int n2 = ToDate.Year * 365 + ToDate.Day;
                for (int i = 0; i < ToDate.Month - 1; i++) 
                { 
                    n2 += monthDays[i]; 
                }
                n2 += countLeapYears(ToDate); 
