...
namespace PayrollWeb.Models
{
    public class SalaryDeductionProcess : ISalaryProcess
    {
        ...
        private List<prl_deduction_configuration> configurationBased;
        ...
        public SalaryDeductionProcess(DateTime processMonthYear,DateTime processMonthYearStartDate,DateTime processMonthYearEndDate, List<prl_employee_details> lstEmployeeSalaryToProcess)
        {
            ...
            //
            thisMonthsDiscontinued = new List<prl_employee_discontinue>();

            //
             thisMonthUploadDeduction = new List<prl_upload_deduction>();
            //
            configurationBased = new List<prl_deduction_configuration>();
        ...
        public IProcessResult Process()
        {
            using (contxt = new payroll_systemContext())
            {
                ...
                //4. discontinued employees within the middle of the month

                ////

                thisMonthsDiscontinued = contxt.prl_employee_discontinue.AsEnumerable().Where(x => x.with_salary == "Y" &&
                        x.discontinue_date.ToString("yyyy-MM") == processMonthYear.ToString("yyyy-MM")).ToList();
               ...
               thisMonthUploadDeduction = contxt.prl_upload_deduction.Include("prl_employee").AsEnumerable()
                ...
                configurationBased = contxt.prl_deduction_configuration.Include("prl_deduction_name").AsEnumerable().
                ...
                //calculate configuartion based deductions

                CalculateGeneralTypeDedutions(configurationBased);
                ...
                try
                {
                    ...
                    foreach (var x in extEmpDetails)
                    {
                        try
                        {
                            ...
                            var sal = SalaryCalculationHelper.CalculateDeduction(daysWorked, proStartDate,proEndDate, x.prl_employee, x, alwConf, sr);
        ...
        private void CalculateUploadedDeduction(List<int> empIds)
        {
            ...
            {
                    // For Implementing and calculating Unpaid_leave_amount

                    //decimal basic_salary = d.prl_employee.prl_employee_details.Select(x => x.basic_salary).First();

                    //decimal unpaid_leave_amount = 0;

                    //if (d.deduction_name_id == 1) // Unpaid Leave (LWP)
                    //{
                //    unpaid_leave_amount = d.amount * (basic_salary / daysInMonth); // here amount is number of unpaid leave days 
                    //}
                    ...
                    try
                    {
                        ...
                        sa.salary_month = d.salary_month_year.Value.Date.AddMonths(1).AddDays(-1);
                        sa.calculation_for_days = daysInMonth;
                        sa.amount = d.amount;
                        sa.arrear_amount = 0;
                        sa.remarks = d.remarks;
                        lstSalAl.Add(sa);
                    }
                    catch (Exception ex)
                    {
