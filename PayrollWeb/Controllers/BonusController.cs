...
namespace PayrollWeb.Controllers
{
    public class BonusController : Controller
    {
        ...
        public ActionResult ConfigureBonus(int bonusId = 0)
        {
            ...
            else
            {
                ...
                if (dbVal == null)
                {
                    ...
                    dbVal.effective_from = DateTime.Now;
                }
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (!errorFound)
                    {
                        ...
                        if (prlConf.created_by == null)
                        {
                            prlConf.created_by = User.Identity.Name;
                        }
                        if (prlConf.created_date == null)
                        {
                            prlConf.created_date = DateTime.Now;
                        }
        ...
        public ActionResult BonusProcess()
        {
            ...
            DateTime bonusProcessDate = CommonDateClass.LastDateForCurrentMonth(nowDateTime).AddMonths(-1);
            _processBonus.process_date = bonusProcessDate;
            _processBonus.festival_date = new DateTime(bonusProcessDate.Year, bonusProcessDate.Month, bonusProcessDate.Day).AddDays(12);
        ...
        public ActionResult BonusProcess(BonusProcess _processBonus)
        {
            ...
            try
            {
                ...
                _result = _service.ProcessBonus(_processBonus, User.Identity.Name);
                if (_result > 0)
                {
                    res.IsSuccessful = true;
                ...
                else
                {
                    ...
                    {
                        res.IsSuccessful = false;
                        res.Message = "Something Worng..!! Bonus has not been processed.";
                    }
                    TempData.Add("msg", res);
                }
            }
        ...
        public ActionResult UndoBonusProcess(BonusProcess _processBonus)
        {
            ...
            try
            {
                ...
                _result = _service.RollbackProcess(_processBonus, User.Identity.Name);
                if (_result == true)
                {
                    res.IsSuccessful = true;
                ...
                {
                    res.IsSuccessful = false;
                    res.Message = "Somthing wrong or Bonus has been disbursed. So not possible to rollbacked.";
                    TempData.Add("msg", res);
                }
            }
            catch (Exception ex)
        ...
        public ActionResult DisburseBonus(string _param = "")
        {
            ...
            else
            {
                ...
                if (_dt != "")
                {
                    ...
                    var dbVal = dataContext.prl_bonus_process.SingleOrDefault(x => x.bonus_name_id == bonusId && x.festival_date.Month == festival_date.Month
                                                                              && x.festival_date.Year == festival_date.Year);
                    if (dbVal == null)
                    {
                ...
                {
                    ModelState.AddModelError("", "This Bonus has not yet been processed. Please process first.");
                }
            }

            var BonusName = dataContext.prl_bonus_name.ToList();
            ViewBag.BonusNames = Mapper.Map<List<BonusName>>(BonusName);
            return View();
        }
